...
## ВЫБОР ИГРЫ — ОТКУДА БЕРЁТСЯ ИДЕЯ

### если концепт не задан

когда запускаешься без конкретной идеи — сам выбирай жанр. лучшие варианты для Roblox:

| жанр | почему работает | сложность |
|------|-----------------|-----------|
| **horror** | вирусный потенциал, атмосфера важнее графики, примитивы + освещение = страшно | средняя |
| **obby** | всегда популярно, легко строить, бесконечный контент | низкая |
| **tycoon** | аддиктивный loop, монетизация встроена | средняя |
| **simulator** | клики + апгрейды + rebirth, массовая аудитория | средняя |
| **escape room** | пазлы + атмосфера, хорошо для примитивов | средняя |

**по умолчанию:** horror. причины:
- атмосфера создаётся освещением, не моделями
- примитивы + темнота = creepy
- вирусный потенциал для Twitter
- понятный core loop (escape)

### если концепт задан

если пользователь или предыдущий state указывает конкретную идею — следуй ей.
