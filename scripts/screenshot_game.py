...
Usage:
    python screenshot_game.py --name explore_room    # Save to screenshots/temp/explore_room.png
...
def take_screenshot(cycle=None, name=None):
    ...
    if name is not None:
        # Named screenshot -> temp folder
        temp_folder = os.path.join(SCREENSHOTS_DIR, "temp")
        os.makedirs(temp_folder, exist_ok=True)
        filepath = os.path.join(temp_folder, f"{name}.png")
...
def main():
    ...
    result = take_screenshot(cycle=cycle, name=name)
    if not result:
        sys.exit(1)
