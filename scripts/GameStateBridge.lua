...
------------------------------------------------------------
-- AGENT COMMAND RELAY: Bridge <-> Player attributes
------------------------------------------------------------

-- Create RemoteEvent for agent results (client -> server)
local agentResultEvent = game.ReplicatedStorage:FindFirstChild("AgentCommandResult")
if not agentResultEvent then
    agentResultEvent = Instance.new("RemoteEvent")
    agentResultEvent.Name = "AgentCommandResult"
    agentResultEvent.Parent = game.ReplicatedStorage
end

-- Handle results from client (AgentControl fires this)
agentResultEvent.OnServerEvent:Connect(function(player, resultJson)
    print("GameStateBridge: Agent result from " .. player.Name .. ": " .. tostring(resultJson):sub(1, 100))

    -- POST result to bridge
    pcall(function()
    ...
    -- Clear command on player
    player:SetAttribute("AgentCommand", "")
end)

-- Command relay loop: poll bridge for commands, set on player
task.spawn(function()
    print("GameStateBridge: Agent command relay started")
    while true do
        task.wait(0.3) -- Check for commands every 0.3s

        local cmdSuccess, cmdResponse = pcall(function()
            return HttpService:GetAsync(BRIDGE_URL .. "/command")
        end)

        if cmdSuccess and cmdResponse then
            local decodeOk, cmdData = pcall(function()
                return HttpService:JSONDecode(cmdResponse)
            end)

            if decodeOk and cmdData and cmdData.command and cmdData.command ~= "" then
                -- Set command on first player
                for _, player in Players:GetPlayers() do
                    local currentCmd = player:GetAttribute("AgentCommand")
                    if not currentCmd or currentCmd == "" then
                        player:SetAttribute("AgentCommand", cmdData.command)
                        print("GameStateBridge: Set AgentCommand on " .. player.Name .. ": " .. cmdData.command)

                        -- Clear command from bridge so it doesn't repeat
                        pcall(function()
                            HttpService:PostAsync(
                                BRIDGE_URL .. "/command",
                                HttpService:JSONEncode({command = ""}),
                                Enum.HttpContentType.ApplicationJson
                            )
                        end)
                        break
